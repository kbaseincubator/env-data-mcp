"""MCP tool functions for the USGS 3DEP elevation adapter."""
...
from ._query import fetch_elevation, to_record
...
@mcp.tool()
def elevation_3dep(*, latitude: float, longitude: float) -> dict[str, Any]:
    """Ground elevation at a point from USGS 3DEP (metres), US only.

    Returns ``{data: [record], _meta}`` — ``{elevation_m, resolution_m, raster_id}``; ``data`` is
    empty (``success: True``) outside 3DEP coverage.  The service is slow (10–15 s is normal);
    ...
    * __latitude, longitude__: The point (WGS84).
    ...
    try:
        ...
        rec = to_record(fetch_elevation(lat=pt.latitude, lon=pt.longitude))
